...
const CommentsPreview = ({ comments, totalCount, onViewAll }) => {
  return (
    <section className="bg-white dark:bg-[#111827] rounded-2xl p-5 border border-gray-100 dark:border-gray-800 shadow-sm dark:shadow-none animate-in fade-in slide-in-from-top-4" aria-label="Comments preview">
      ...
      <div className="flex flex-col gap-3">
        {comments.map((c) => (
          <article key={c.id} className="flex items-start gap-3 bg-[#F9FAFB] dark:bg-[#0B1120] rounded-xl p-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-[#364153] dark:text-gray-200 mt-0.5">{c.text}</p>
            </div>
          </article>
        ))}
      </div>
