...
const DoctorCard = ({ doctor }) => {
  ...
  return (
    <article className="bg-white dark:bg-[#111827] rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm p-5 flex flex-col items-center text-center hover:shadow-md transition">
      <div className="flex flex-col items-center gap-2 flex-1 w-full">
        <img
          src={doctor.img}
          alt={`Avatar of ${doctor.name}`}
          className="w-20 h-20 rounded-full object-cover border-4 border-white shadow mt-1"
          onError={(e) => { e.target.src = `https://ui-avatars.com/api/?name=${doctor.name}&background=E8EAF6&color=333CF5`; }}
        />
        <h3 className="text-[20px] font-bold text-black-main-text dark:text-[#E2E8F0]">{doctor.name}</h3>
        <div className="flex items-start gap-1 text-[14px] text-gray-500 dark:text-gray-400">
          <HiOutlineLocationMarker className="text-gray-500 dark:text-gray-400 shrink-0 mt-0.5" aria-label="Location" />
          <span>{doctor.loc}</span>
        </div>
