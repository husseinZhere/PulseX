...
const PatientAllComments = () => {
  ...
  const userInitials = (user?.fullName || 'P').charAt(0).toUpperCase();
  const [userAvatar, setUserAvatar] = useState(() => readProfilePhoto('patient') || '');

  useEffect(() => {
    if (userAvatar) return;
    getUserProfile()
      .then((profile) => {
        const pic = profile?.profilePicture || profile?.ProfilePicture;
  ...
  if (isLoading) {
    return (
      <main className="w-full flex flex-col gap-5 p-5 text-black-main-text dark:text-[#E2E8F0]">
  ...
  return (
    <>
      ...
      <main className="w-full flex flex-col gap-5 p-5 text-black-main-text dark:text-[#E2E8F0]">
        <AllCommentsHeader
          onBack={() => navigate(`/patient/stories/${id}`)}
